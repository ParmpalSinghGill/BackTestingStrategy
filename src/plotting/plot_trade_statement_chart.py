"""
Trade Candlestick Chart Renderer with Statement & Account Balance Overlay
...
def plot_statement_trade_chart(trade_record: dict, output_dir: Path = PLOTS_DIR) -> str:
    ...
    ax.axhline(sup_price, color="#8b5cf6", linestyle="-", linewidth=2.0, label=f"Liquidity Source ({liquidity_source}): ₹{sup_price:.2f}")
    ax.axhline(entry_p, color="#3b82f6", linestyle="--", linewidth=1.8, label=f"Entry Price: ₹{entry_p:.2f} ({c2_date_str})")
    ax.axhline(sl_p, color="#ef4444", linestyle="--", linewidth=1.8, label=f"Stop Loss (SL): ₹{sl_p:.2f}")
    ax.axhline(tp_p, color="#10b981", linestyle="--", linewidth=1.8, label=f"Target ({target_rr_label} RR): ₹{tp_p:.2f}")
    ...
    info_text = (
        ...
        f"-------------------------------\n"
        f"• Ticker: {symbol}\n"
        f"• Liquidity Level: {liquidity_source} (₹{sup_price:.2f})\n"
        f"• Entry: {c2_date_str} @ ₹{entry_p:.2f}\n"
        f"• Exit:  {exit_date_str} @ ₹{actual_exit_price:.2f}\n"
        f"• Target RR: {target_rr_label} RR (SL: ₹{sl_p:.2f} | TP: ₹{tp_p:.2f})\n"
        f"• Quantity: {trade_record['Quantity']:,} shares\n"
        f"• Total Spend (Buy): ₹{total_spend:,.2f}\n"
        f"• Balance After Buy: ₹{bal_after_entry:,.2f}\n"
        f"• Net Trade PnL: {pnl_str} ({return_pct:+.2f}%)\n"
        f"• Balance After Exit: ₹{bal_after_exit:,.2f}"
    )
