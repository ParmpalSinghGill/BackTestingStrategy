...
def main():
    ...
    print(f"Loaded {len(trades_df)} trades. Generating categorized plots...")
    ...
    for ticker, trades in grouped:
        ...
        # Cache resampled dataframes for this stock
        resampled_cache = {}
        ...
        for _, trade in trades.iterrows():
            ...
            # Use cached resampled dataframe if available
            if tf not in resampled_cache:
                df_tf = resample_to_timeframe(raw_df, tf)
                df_tf["dt"] = pd.to_datetime(df_tf["dt"])
                resampled_cache[tf] = df_tf
                
            df = resampled_cache[tf]
            ...
            # Categorize into folders: e.g. Pivot_15M, PrevDay_15M, Combined_15M
            sources = str(trade["level_sources"]).split(",")
            is_pivot = any("pivot" in s for s in sources)
            is_prev_day = any("prev_day" in s for s in sources)
            tf_upper = tf.upper()
            
            if is_pivot and is_prev_day:
                folder_name = f"Combined_{tf_upper}"
            elif is_prev_day:
                folder_name = f"PrevDay_{tf_upper}"
            else:
                folder_name = f"Pivot_{tf_upper}"
                
            trade_plots_dir = PLOTS_DIR / folder_name
            os.makedirs(trade_plots_dir, exist_ok=True)
            
            # Save plot
            file_name = f"{ticker}_{trade_date}_{tf}_pass{trade['pass_no']}.png"
            fig.savefig(trade_plots_dir / file_name, dpi=120)
            plt.close(fig)
            total_plots += 1
            
    print(f"\nDone! Generated {total_plots} categorized plots under: {PLOTS_DIR}")
