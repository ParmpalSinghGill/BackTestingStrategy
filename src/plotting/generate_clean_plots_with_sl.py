"""
Generate Clean, High-Resolution OHLC Trade Plots with Initial SL Line & Legend Box
----------------------------------------------------------------------------------
1. Clean candlestick body rendering with minimal text clutter.
2. Dotted Maroon Line for Initial Stop Loss (SL).
3. Shifted trade details to an elegant Legend Info Card.
"""
...
def plot_candlestick_trade_chart(day_df: pd.DataFrame, stock_name: str, trade: dict, range_high: float, range_low: float):
    ...
    # ENTRY MARKER (CLEAN COMPACT CALLOUT)
    entry_t = pd.to_datetime(trade['Entry_Time'])
    entry_time_str = entry_t.strftime("%H:%M")
    ax.scatter(entry_t, entry_p, color=color_side, s=140, zorder=7, marker=marker_side)
    ...
    info_text = (
        ...
        f"Exit 1: {ex1_t.strftime('%H:%M')} @ {ex1_p:.2f} ({trade['Exit1_PnL_Pct']:+.2f}%)\n"
        f"Exit 2: {ex2_t.strftime('%H:%M')} @ {ex2_p:.2f} ({trade['Exit2_PnL_Pct']:+.2f}%)\n"
    ...
    # Combine lines legend + info card text box
...
def main():
    ...
    print(f"Loaded {len(trades_df)} trades from {trades_csv.name}.")
    ...
    print(f"Successfully re-generated {count} CLEAN trade plots with Initial SL Line & Legend Card in {PLOTS_DIR}!")
