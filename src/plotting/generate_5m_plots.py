"""
Generate 5-Minute OHLC Candlestick Trade Plots in plot/Plot_5M/
"""
...
PLOTS_DIR = BASE_DIR / "plot" / "Plot_5M"
...
def draw_candlesticks(ax, df_candles):
    ...
    for idx, row in df_candles.iterrows():
        ...
        ax.vlines(t, l, h, color=color, linewidth=1.1, zorder=3)
        body_bottom = min(o, c)
...
def plot_candlestick_trade_chart(day_df: pd.DataFrame, stock_name: str, trade: dict, range_high: float, range_low: float):
    ...
    plot_5m = resample_ohlc(day_df.between_time("09:15", "15:30"), '5min')
    if plot_5m.empty:
        plt.close(fig)
        return
        
    draw_candlesticks(ax, plot_5m)
    ...
    range_start = plot_5m.index[0].replace(hour=9, minute=30, second=0)
    range_end = plot_5m.index[0].replace(hour=9, minute=45, second=0)
    ...
    title_text = f"[5M STRATEGY] {stock_name} ({date_str}) | Side: {side} | Net PnL: {pnl_str} ({trade['Net_Trade_PnL_Pct']:+.2f}%) [{trade['Trade_Status']}]"
...
def main():
    trades_csv = REPORTS_DIR / "5M_Top5_Trades_Detailed.csv"
    ...
    print(f"Successfully generated {count} 5-Minute trade plots in {PLOTS_DIR}!")
