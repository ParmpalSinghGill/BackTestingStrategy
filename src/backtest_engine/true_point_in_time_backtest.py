"""
Strict Minute-by-Minute Point-in-Time Backtesting Engine
--------------------------------------------------------
No forward-looking bias:
1. Calculates 9:30-9:45 Range at 09:45 AM.
2. Selects Top 5 Volume stocks per day (Range Width 0.4%-1.8%).
3. Checks 15M Confirmation at 15m boundaries (10:00, 10:15, etc.).
4. Places Limit Order at 9:30 Level.
5. EXECUTES ENTRY ONLY WHEN A SUBSEQUENT MINUTE CANDLE RETESTS THE LIMIT PRICE!
6. Manages SL (0.3%), TP1 (1.0%/opposite), TP2 (2.0%), and EOD Exit (15:10 PM) minute-by-minute.
"""
...
def plot_candlestick_trade_chart(day_df: pd.DataFrame, stock_name: str, trade: dict, range_high: float, range_low: float):
    ...
    ax.scatter(conf_t, conf_p, color='#3498db', s=120, zorder=6, marker='d')
    ax.annotate(f"CONFIRMATION (15M)\nTime: {conf_time_str}", (conf_t, conf_p),
                textcoords="offset points", xytext=(0, 25 if side == 'SHORT' else -35),
                ha='center', fontsize=8, fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.3", fc="#ebf5fb", ec="#3498db", lw=1.2))

    # 2. RETEST ENTRY MARKER & LABEL
    ...
    title_text = f"[TRUE BACKTEST] {stock_name} ({date_str}) | Side: {side} | Retest Entry @ {entry_time_str} | Net PnL: {pnl_str} ({trade['Net_Trade_PnL_Pct']:+.2f}%) [{trade['Trade_Status']}]"
...
def run_point_in_time_backtest():
    ...
    print(f"Loaded {len(csv_files)} stock data CSV files for Point-In-Time Backtesting.\n")
    ...
    for filepath in csv_files:
        ...
        for day_date, day_df in df.groupby(df.index.date):
            ...
            # STEP 1: Point-in-time check at 09:45 AM for 09:30-09:45 Range
    ...
    # STEP 2: Rank & Select Top 5 Volume Stocks Per Day at 09:45 AM
    top5_per_day = filtered_cand.groupby('Date', group_keys=False).apply(lambda g: g.nlargest(5, 'Range_Volume')).reset_index(drop=True)
    
    print(f"Selected Top 5 Stocks across {top5_per_day['Date'].nunique()} Trading Days ({len(top5_per_day)} total stock-days).\n")
    
    executed_trades = []

    # STEP 3: Minute-by-Minute Point-In-Time Simulation
    for _, item in top5_per_day.iterrows():
        ...
        # 1-minute data from 09:45 onwards
        df_eval = day_df.between_time("09:45", "15:15")
        if df_eval.empty: continue
        
        minute_records = df_eval.reset_index().to_dict('records')
        
        pending_orders = [] # Active Limit Orders placed after 15M confirmation
        long_done = False
        short_done = False
        
        # 15M Candle Boundaries for Confirmation
        # Check confirmation at 10:00, 10:15, 10:30, 10:45, 11:00, ..., 14:45
        
        for idx, m_row in enumerate(minute_records):
            m_time = m_row['Datetime']
            m_time_str = m_time.strftime("%H:%M")
            
            # 1. AT 15-MINUTE BOUNDARIES (10:00, 10:15, etc.), CHECK FOR 15M REVERSAL CONFIRMATION
            if m_time_str in ["10:00", "10:15", "10:30", "10:45", "11:00", "11:15", "11:30", "11:45",
                              "12:00", "12:15", "12:30", "12:45", "13:00", "13:15", "13:30", "13:45",
                              "14:00", "14:15", "14:30", "14:45"]:
                              
                # Extract the 15-minute candle that just completed
                c_15m_start = m_time - pd.Timedelta(minutes=15)
                c_15m_df = day_df[(day_df.index >= c_15m_start) & (day_df.index < m_time)]
                
                if not c_15m_df.empty:
                    c_15m_open = c_15m_df['Open'].iloc[0]
                    c_15m_high = c_15m_df['High'].max()
                    c_15m_low = c_15m_df['Low'].min()
                    c_15m_close = c_15m_df['Close'].iloc[-1]
                    
                    # LONG Confirmation: Touched Low, closed Green and above Low
                    if not long_done and c_15m_low <= range_low * 1.001 and c_15m_close > range_low and c_15m_close >= c_15m_open:
                        pending_orders.append({
                            'Side': 'LONG', 'Limit_Price': range_low, 'Conf_Time': m_time, 'Conf_Idx': idx
                        })
                        long_done = True
                        
                    # SHORT Confirmation: Touched High, closed Red and below High
                    if not short_done and c_15m_high >= range_high * 0.999 and c_15m_close < range_high and c_15m_close <= c_15m_open:
                        pending_orders.append({
                            'Side': 'SHORT', 'Limit_Price': range_high, 'Conf_Time': m_time, 'Conf_Idx': idx
                        })
                        short_done = True

            # 2. MINUTE-BY-MINUTE CHECK FOR RETEST LIMIT ORDER EXECUTION
            orders_to_remove = []
            for order in pending_orders:
                # Retest occurs ONLY after confirmation minute
                if idx > order['Conf_Idx']:
                    side = order['Side']
                    ...
                    if retest_triggered:
                        # RETEST EXECUTION CONFIRMED! Simulate position management from this minute
                        entry_t = m_time
                        entry_p = limit_p
                        
                        if side == 'LONG':
                            sl_p = limit_p * 0.997
                            tp1_p = min(limit_p * 1.010, range_high)
                            tp2_p = limit_p * 1.020
                        else:
                            sl_p = limit_p * 1.003
                            tp1_p = max(limit_p * 0.990, range_low)
                            tp2_p = limit_p * 0.980
                        ...
                        gross_pnl_amount = (0.5 * (ex1_p - entry_p) + 0.5 * (ex2_p - entry_p)) * qty if side == 'LONG' else (0.5 * (entry_p - ex1_p) + 0.5 * (entry_p - ex2_p)) * qty
                        ...
                        tr_record = {
                            'Date': date_str, 'Stock_Name': stock_name, 'Ticker': ticker, 'Side': side,
                            'Range_930_High': round(range_high, 2), 'Range_930_Low': round(range_low, 2),
                            'Confirmation_Time': order['Conf_Time'], 'Retest_Entry_Time': entry_t,
                        ...
                        executed_trades.append((tr_record, day_df, stock_name, range_high, range_low))
                        orders_to_remove.append(order)

            for ord_rem in orders_to_remove:
                pending_orders.remove(ord_rem)

    if not executed_trades:
        print("No point-in-time trades executed.")
        return
    ...
    detailed_csv = REPORTS_DIR / "Strict_True_Backtest_Trades_Detailed.csv"
    trades_df.to_csv(detailed_csv, index=False)
    print(f"Saved Strict Point-In-Time Trade Log ({len(trades_df)} trades) -> {detailed_csv}")
    ...
    daily_csv = REPORTS_DIR / "Strict_True_Backtest_Daily_PnL_Summary.csv"
    daily_df.to_csv(daily_csv, index=False)
    print(f"Saved Strict Point-In-Time Daily PnL Summary ({len(daily_df)} days) -> {daily_csv}\n")

    # Generate OHLC Candlestick Charts with Confirmation & Retest Entry Markers
    print(f"Generating Strict Retest Candlestick Trade Plots in {PLOTS_DIR}...")
    ...
    print("\n==========================================================================")
    print("      STRICT MINUTE-BY-MINUTE POINT-IN-TIME BACKTEST RESULTS               ")
    print("==========================================================================")
