"""
Multi-Tier Risk-Reward ML Selector Strategy & Master Benchmark

Trains Multi-Class Random Forest Model to dynamically classify setups into:
- SkipTrade
- 1:2 RR Target
- 1:3 RR Target
- 1:4 RR Target
...
Evaluates portfolio performance starting from Rs 100,000 INR (2010 to 2026) net after all Indian statutory taxes.
"""
...
from src.analysis.multi_tier_rr_evaluator import build_multi_tier_dataset


def run_walk_forward_multi_tier_model(df_input: pd.DataFrame, probability_threshold: float = 0.40) -> pd.DataFrame:
    cached_df = load_cached_predictions("rf_multi_tier", "multitier", probability_threshold)
    ...
    for test_year in years:
        ...
        y_train = train_df["Multi_Tier_Label"].values
    ...
    save_predictions_to_cache(df_accepted, "rf_multi_tier", "multitier", probability_threshold)
    return df_accepted


def simulate_multi_tier_portfolio(
...
def run_master_multi_tier_benchmark():
    print("=========================================================================")
    print("MULTI-TIER RISK-REWARD ML SELECTOR BENCHMARK (1:2 to 1:15 Targets)")
    print("=========================================================================\n", flush=True)

    df_mt = build_multi_tier_dataset()

    print("Running Walk-Forward ML Training for Multi-Tier RR Selection...", flush=True)
    df_acc = run_walk_forward_multi_tier_model(df_mt, probability_threshold=0.40)

    res_zero = simulate_multi_tier_portfolio(df_acc, starting_capital=100000.0, flat_brokerage_per_order=0.0)
    res_flat20 = simulate_multi_tier_portfolio(df_acc, starting_capital=100000.0, flat_brokerage_per_order=20.0)

    report_row = [{
        "Strategy & Model Variant": "Multi-Tier Dynamic ML RR Selector (SkipTrade, 1:2 to 1:15 RR)",
    ...
    print("\n==========================================================================================================")
    print("MULTI-TIER DYNAMIC RISK-REWARD ML SELECTOR PERFORMANCE SUMMARY")
    print("==========================================================================================================")
    print(df_rep.to_string(index=False), flush=True)

    out_csv = REPORTS_DIR / "Multi_Tier_ML_RR_Selector_Comparison_Results.csv"
