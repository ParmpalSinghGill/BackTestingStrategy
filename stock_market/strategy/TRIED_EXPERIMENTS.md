...
| M49 | Swing_PP live-valid entry/SL/RR hunt. C2 buy-stop at C1 high (gap=open vs wait; skip if same-bar SL while open still below trigger). Rank at C1 close (no C2 close). Tighter SL = C1×0.99. C3 limit at C1 high (no chase). Failed-breakout scratch: C3 close back below C1 high. Condition 1:3 after +1R (close loc≥0.60, high<1.8R). | **C2-cross C1-close rank: dead.** Tight SL **+3.3% / 55% DD** (n=262); sweep SL **+0.5% / 50% DD** (n=144). C3 limit / wait-for-pullback **+6 to +19% / 43–54% DD** (runners never come back). C1×0.99 at C3 open **+50.8% / 42% DD** (2% of equity on a tiny stop over-sizes). Condition 1:3 OOS hit **43%** vs 67% needed to beat banking 2R: **+48.8% / 24.2% DD**. Failed-breakout scratch **+59.32% / 24.7% DD**. Next-bar ML **+57.09% / 24.5%**. Scratch+ML **+59.92% / 24.7%**. | Do not enter on C2 cross. Do not tighten SL to C1 without a risk-pct floor. Do not wait for C3 pullback to C1 high. Do not raise TP on a close-location rule. **24.7% DD accepted** — C3-close scratch + next-bar ML SHIFT is on 4pm Swing_PP (`Swing_PP_ins.txt`). Not Swing_low / Swing_Live. |
