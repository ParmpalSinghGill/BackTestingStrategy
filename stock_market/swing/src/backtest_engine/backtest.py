...
MAX_RISK_PCT = 0.75     # if SL is more than this % away, don't take trade (0.75% max loss)
ALLOWED_TRIES = 1       # max consecutive tries on level cross (re-entries)
...
def next_available_date(target_date, available_dates) -> object:
    ...
    if later:
        next_date = later[0]
        if (next_date - target_date).days <= 3:
            return next_date
    return None
...
def run_setup(day, cross_idx, direction, level_name, level_val, pass_no, stock,
    ...
    risk_pct = abs(initial_stop - entry) / entry * 100.0
    if MAX_RISK_PCT is not None and risk_pct > MAX_RISK_PCT:
        return None, "max_risk_exceeded", entry_idx
...
def main():
    ...
    print(f"\nSetups that did not trade -> "
          ...
          f"max_risk_exceeded: {diag['max_risk_exceeded']}")
