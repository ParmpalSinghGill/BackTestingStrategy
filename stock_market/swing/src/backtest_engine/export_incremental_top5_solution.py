"""
Export Incremental 1-Minute Live Simulation CSVs & Generate OHLC Candlestick Plots
----------------------------------------------------------------------------------
Includes explicit annotations for:
1. CONFIRMATION CANDLE (Blue Diamond) - 15M Close where direction is decided.
2. LIMIT ENTRY CANDLE (Green/Red Triangle) - Minute where Limit Order is filled.
3. EXIT 1 CANDLE (Yellow Circle) - 50% Partial Exit.
4. EXIT 2 CANDLE (Purple Square) - 50% Final Exit.
"""
...
def plot_candlestick_trade_chart(day_df: pd.DataFrame, stock_name: str, trade: dict, range_high: float, range_low: float):
    ...
    # 1. CONFIRMATION CANDLE MARKER & LABEL
    ...
    # 2. LIMIT ENTRY CANDLE MARKER & LABEL
    ...
    title_text = f"[INCREMENTAL ENGINE] {stock_name} ({date_str}) | Side: {side} | Conf: {conf_time_str} | Entry: {entry_time_str} | Net PnL: {pnl_str} ({trade['Net_Trade_PnL_Pct']:+.2f}%) [{trade['Trade_Status']}]"
...
def main():
        ...
        for t in time_range:
            ...
            if t_str in ["10:00", "10:15", "10:30", "10:45", "11:00", "11:15", "11:30", "11:45",
                ...
                for sname in top5_names:
                    ...
                    df_up_to_t = sinfo['DF'][sinfo['DF'].index < t]
                    ...
                    if not c_15m_df.empty:
                        ...
                        if side:
                            ...
                            conf_t = t # Confirmation 15M Close time
                            entry_t = t # Limit order execution time
                            ...
                            tr_rec = {
                                ...
                                'Confirmation_Time': conf_t, 'Entry_Time': entry_t, 'Entry_Price': round(entry_p, 2),
    ...
    print(f"Saved Incremental 1M Trade Log ({len(trades_df)} trades) -> {detailed_csv}")
    ...
    print(f"Saved Incremental Daily PnL Summary ({len(daily_df)} days) -> {daily_csv}\n")
