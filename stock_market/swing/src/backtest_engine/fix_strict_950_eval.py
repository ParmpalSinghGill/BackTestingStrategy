"""
Fix Strict 09:50 AM Entry Cutoff in Resampled 15M Data
------------------------------------------------------
Enforces that NO TRADE can enter before 10:00 AM (or strict 09:50 AM).
Saves updated CSVs and regenerates all plots in plot/Plot_15M/.
"""
...
def main():
    ...
    for filepath in csv_files:
        ...
        for day_date, day_df in df.groupby(df.index.date):
            ...
            # RESAMPLE TO 15M AND FILTER STRICTLY CANDLE TIME >= 10:00 AM (FIRST 15M CANDLE AFTER 9:45)
            ...
            for i, c in enumerate(candles):
                ...
                # STRICT REQUIREMENT: Candle Datetime MUST BE >= 10:00 AM (NO 09:45 CANDLE ENTRY)
                if "10:00" <= c_time_str <= "14:45":
    ...
    print(f"Saved Strict >= 10:00 AM Entry Trade Log ({len(top5_df_clean)} trades) -> {detailed_csv}")
    ...
    print(f"Saved Strict >= 10:00 AM Daily PnL Summary ({len(daily_df)} days) -> {daily_csv}")

    # Clear old plots & generate new plots
    ...
    print(f"Successfully re-generated {plot_count} trade plots in {PLOTS_DIR} with STRICT >= 10:00 AM Entry!")
