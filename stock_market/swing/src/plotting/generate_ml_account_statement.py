"""
Streamlined 3-Class Dynamic ML Strategy Account Statement Generator (Initial Deposit: Rs 100,000)

Matches the exact benchmark simulation logic from src.analysis.run_streamlined_multi_class_benchmark
starting from Rs 100,000 initial capital deposit.

Generates:
1. Chronological Buy/Exit trade log with running capital balance updates.
2. High-resolution PNG candlestick charts for trades with Support Level, Entry, Exit, SL, Target, and PnL text overlays.
3. Formatted Excel Account Statement (`Streamlined_ML_Strategy_Account_Statement_100k.xlsx`) with hyperlinked local chart URIs.
"""
...
from src.plotting.plot_trade_statement_chart import plot_statement_trade_chart


def run_account_statement_generation(initial_deposit: float = 100000.0, max_charts_to_generate: int = 50):
    print(f"=== Streamlined 3-Class Dynamic ML Strategy Statement Engine (Initial Deposit: Rs {initial_deposit:,.2f}) ===", flush=True)
    ...
    print(f"Loaded {len(df_acc):,} accepted trade signals for chronological benchmark simulation.", flush=True)
    ...
    for curr_dt in all_days:
        ...
        for pos in closed_positions:
            ...
            # Chart generation (for first N trades to optimize performance)
            chart_uri = "N/A"
    ...
    print(f"\nCompleted Benchmark Statement Simulation!")
    ...
    # 3. Export to Excel with Hyperlinks and Formatted Sheets
...
if __name__ == "__main__":
    run_account_statement_generation(initial_deposit=100000.0, max_charts_to_generate=50)
